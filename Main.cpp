...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    name_to_var->insert({"warp", var11});

    auto * value_server = new unordered_map<string, double >();
    auto * sim_to_var = new unordered_map<string, Var*>();

    auto * symbol_table = new SymboleTable(value_server,  name_to_var, sim_to_var);

    SleepCommand sleep_command(symbol_table);
    fromTape->insert({"Sleep", &sleep_command});

    PrintCommand print_command(symbol_table);
    fromTape->insert({"Print", &print_command});

    ChangeValue change_value(symbol_table, 1);
    fromTape->insert({"changeValue", &change_value});

    auto * func_parser = new FuncParser;
    fromTape->insert({"funcParser", func_parser});

    IfCommand if_command(fromTape, symbol_table, func_parser);
    fromTape->insert({"if", &if_command});

    LoopCommand loop_command(fromTape, symbol_table, func_parser);
    fromTape->insert({"while", &loop_command});

    FuncCommand func_command(symbol_table, fromTape, func_parser);
...
vector <string> lexer(const string& file_name) {
  ...
  while (getline(file, line)) {
    ...
    for (unsigned long i = 0; i < line.length(); i++) {
      ...
      if (line[i] == '(') {
        //indexCloser = i;
        indexCloser = line.find_last_of(')', line.length() - 1);
        if ((line[i + 1] == line[indexCloser - 1]) && (line[i + 1] == '"')) {
          isString = true;
        }
        ...
        if ((counterComa >= 1) && (!isString)) { //more that 1 param to the func
...
void parser(vector <string> tokens, unordered_map<string, Command*> * fromTape,
  ...
  int i = 124; //change - since we are not using the first 3 commands(server, client, var creation)
  while (i < tokens.size()) {
    Command* c;
